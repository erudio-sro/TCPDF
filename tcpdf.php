...
class TCPDF {
   ...
   /**
    * @var bool Erudio - povolení nedělitelných mezer. Při true dělá bordel - nedoporučuje se !!!
    */
	protected $enableNbsp = false;

   /**
    * @var bool Erudio - zda bylo falešně podepsáno - s 00000 podpisem
    */
   ...
   */
   private $alternateFontFiles = self::DEFAULT_ALTERNATE_FONTFILES;
	...
	public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='', $stretch=0, $ignore_min_height=false, $calign='T', $valign='M') {
      ...
      $txt = (string) $txt; //Erudio: php 8.x, aby tam šlo dát null
	...
	public function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $ln=1, $x='', $y='', $reseth=true, $stretch=0, $ishtml=false, $autopadding=true, $maxh=0, $valign='T', $fitcell=false) {
      ...
      if($align == 'J') //Erudio oprava posledního řádku v zarovnání do bloku
      {
            $txt .= "\n";
      }
      $txt = str_replace(chr(9),"",$txt); //Erudio
      ...
      {
            $txt = str_replace(chr(0xc2) . chr(0xa0), " ", $txt); //Erudio nedělitelná mezera, dělá bordel, odstranit
      }
   ...
   /** Erudio
			...
			if ($handle = @opendir(K_PATH_CACHE)) {
				while ( false !== ( $file_name = readdir( $handle ) ) ) {
					if (strpos($file_name, '__tcpdf_'.$this->file_id.'_') === 0) {
						@unlink(K_PATH_CACHE.$file_name); //Erudio: at sign
					}
				}
				closedir($handle);
			}
	...
	protected function getHtmlDomArray($html) {
		...
		$csstagarray = '<cssarray>'.htmlentities(json_encode($css),
                   /*Erudio: doplnění UTF-8, kvůli tomu, že výchozí kódování u nás je 1250 */
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
   ...
   /**
    ...
    protected function fileExists($file)
    {
     ...
     /**
      * Funkce pro zjistění počtu řádků vzniklých dělením multicellu
      * @param int $width
      * @param string $txt
      * @return array ('num_line' => počet řádek, 'txt_len' => délka textu celkem)
      */
            ...
            elseif ( $h >= 0xC2 ) {
               ...
               elseif ( ($h <= 0xF4) && ($i < $len -3) )
                  $out[] = ($h & 0x0F) << 18 | (ord($str[++$i]) & 0x3F) << 12
                                             | (ord($str[++$i]) & 0x3F) << 6
                                             | (ord($str[++$i]) & 0x3F); //Erudio: opravy [++$i] místo {++$i} kvůli PHP8
            }
         }
         return $out;
      }
   ...
   /** Erudio
    * přidá td do řádku tabulky na daný počet sloupců
    * @param DOMNode $tr Řádek
    * @param int $num Počet sloupců kolik má být
   ...
   /**
    * !!! POZOR - NEFUNKČNÍ V SIS4 !!!
    * 
    * Vypíše text s informací, že PDF je podepsané. Na text se dá v Adobe readeru kliknout - zobrazí certifikát
    * @param string $txt IGNOROVANO. Text s informací o podepsání. Při null se bere výchozí jaderný.
    * @param array $cert_cfg Konfigurační parametr s certifikátem. Pokud není uveden, bere se vyspl/cert dle fakulty z dl_ses
    ...
    * !!! POZOR - NEFUNKČNÍ V SIS4 !!!
    * 
    * Opatří PDF certifikační značkou
    * @param string $reason Důvod podepsání (zobrazuje se v popisu certifikátu).
    * @param array $cert_cfg Konfigurační parametr certifikátu. Při null se bere z vyspl/cert dle fakulty z dl_ses
    * @param string $fakulta Fakulta, pokud neuvedeno, pak fakulta z dl_ses
    */
   ...
   /**
    * Erudio: Snaha o opravu chyby, kdy počet sloupců tabulky se počítá z prvního řádku
    * -> doplní do tabulek do prvního řádku buňky
    * @param string $html
    * @return string
    */
      ...
      /** Erudio - povolení nedělitelných mezer. POZOR: může způsobovat problémy
      ...
      /**
       * Erudio: Aktuální font může zobrazit text?
       * @param string $text
       * @return bool
       */
      public function canDisplayText($text) : bool
      {
            ...
            {
                  if ($c === 13 || $c === 10) //Odřádkování ignoruji
                  {
                        continue;
                  }
                  if (!isset($this->CurrentFont['cw'][$c]))
       ...
       * @return bool True pokud se ve finále podařilo nastavit font co zobrazí celý text
       */
